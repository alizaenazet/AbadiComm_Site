...
gap="18px" title="admin login">
    ...
    <h1 class="text-h1-sm font-medium ">Update Team member</h1>
    <form class="w-full h-fit flex flex-col items-center gap-3 justify-center" action="/dashboard/team-member/{{$member->id}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <input type="hidden" id="updated" name="updated" value="">
    <div  class="w-full md:w-[348px] h-fit ">
            <img id="image-preview" class="w-full h-fit aspect-square drop-shadow-md rounded-sm" src={{$member->image_url}} alt="" >
    ...
    <button id="reset-image-preview" type="button" class="w-fit text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-input-sm md:text-input-lg px-[10px] py-[5px]  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" >Ganti gambar</button>
        <div class="flex flex-col items-start gap-2">
            ...
            <div class="w-full h-fit">
                ...
                <option selected value={{$member->division->id}}>{{$member->division->name}}</option>
                @foreach ($divisions as $division)
                    @if ($division->id != $member->division->id)
    ...
    <button type="submit" class="w-full md:w-[160px] text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-input-sm md:text-input-lg px-[10px] py-[5px]  dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Update member</button>
    </form>

    <script>
        $(document).ready(function () {
            var defaultNameInput = $('#name-input').val();
            var defaultQualificationInput = $('#qualification-input').val();
            var defaultDevisionValue = $('#divisions').val();
            var updated = [];
            ...
            $('#name-input').change(function (e) { 
                e.preventDefault();
                if ($('#name-input').val() != defaultNameInput) {
                    if (!updated.includes('name')) {
                        updated.push('name')
                        $('#updated').val(updated.toString());
                    }
                }else{
                    const indexOfValue = updated.indexOf("name");
                    if (updated.includes('name')) {
            ...
            $('#divisions').change(function (e) { 
                e.preventDefault();
                if ($('#divisions').val() != defaultDevisionValue) {
                    if (!updated.includes('division')) {
                        updated.push('division')
                        $('#updated').val(updated.toString());
                    }
                }else{
                    const indexOfValue = updated.indexOf("division");
                    if (updated.includes('division')) {
                        updated.splice(indexOfValue,1)
                        $('#updated').val(updated.toString());
                    }
            ...
            $('#qualification-input').change(function (e) { 
                e.preventDefault();
                if ($('#qualification-input').val() != defaultQualificationInput) {
                    if (!updated.includes('qualification')) {
                        updated.push('qualification')
                        $('#updated').val(updated.toString());
                    }
                }else{
                    const indexOfValue = updated.indexOf("qualification");
                    if (updated.includes('qualification')) {
                        updated.splice(indexOfValue,1)
                        $('#updated').val(updated.toString());
                    }
