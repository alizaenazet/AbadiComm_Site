<x-app-layout 
    ...
    <div id="galleryList" class="w-full h-fit flex flex-row flex-wrap gap-3 justify-center">
        @foreach ($galleries as $gallery)
        <img id={{$gallery->id}} class=" imageList w-[164px] h-[164px] sm:w-[100px] sm:h-[100px] rounded-md hover:-translate-y-1 hover:scale-500" src={{ $gallery->image_url }} alt={{ $gallery->content }}>
        @endforeach
    </div>
            ...
            <div class=" flex justify-center relative transform overflow-hidden rounded-lg  text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                    <div  class="flex flex-col flex-wrap justify-center items-center p-[15px] h-fit w-fit bg-gray-300 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-30 border border-gray-100 gap-4">
                        ...
                        <div  class="w-full flex flex-end mb-2">
                            <h2 id="close-modal-button" class="font-bold text-lg px-[7px] rounded-full hover:bg-accent bg-white ">X</h2>
                            </div>
                            <div class=" w-[284px] h-[284px]" style="background-image: url(${imageUrl})"></div>
                            </div>
                            <p>${description}</p>
                    </div>
            </div>
